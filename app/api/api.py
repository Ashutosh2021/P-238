...
def login():
    try:
        ...
        #Query being seperately created and executed creates vulnerability of SQL Injection 
...
def create_order():
    try:
        ...
        # Creating and executing queries seperately makes the codes vulnerable
        user_query = f"select * from users where email='{user_email}';"
        user = db.engine.execute(user_query).first()
...
def submit_help():
    ...
    #The type of the file being submitted should be checked in order to avoid Phishing Attacks
    if attachment:
        filename = secure_filename(attachment.filename)
        attachment.save(os.path.join(UPLOAD_FOLDER, filename))

    # Creates Vulnerability of SQL Injection
    user_email = session.get("email")
    user_query = f"select * from users where email='{user_email}';"
    user = db.engine.execute(user_query).first()
